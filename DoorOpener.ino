...
//#include <time.h> // Too much memory usage
...
// Trying to save up RAM
const PROGMEM byte months[11] = {31, 28 , 31, 30, 31, 30, 31, 31, 30, 31, 30}; 
...
unsigned long now() {   
	// Month lenghts except december
	
	unsigned long epoch = Y2K;
    ...
    epoch += bcdToDec(Wire.read()); // Adding seconds
	epoch += bcdToDec(Wire.read()) * 60; // Adding minutes
	epoch += bcdToDec(Wire.read() & 0b111111) * 3600; //24 hour time
	bcdToDec(Wire.read()); // Weekday don't care
	epoch += (bcdToDec(Wire.read()) - 1) * 86400; // Adding days
	
	byte month = bcdToDec(Wire.read()) - 1;
	for(byte m = 0; m < month; m++) 
		epoch += pgm_read_byte_near(months + m) * 86400; // Adding months 
	
	epoch += bcdToDec(Wire.read()) * 31536000; // Adding years
